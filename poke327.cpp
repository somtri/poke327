...
void init_pc()
{
  ...
  world.pc.money = 500;
  world.pc.storage_count = 0;
  for (int i = 0; i < 400; i++) {
    world.pc.storage[i] = NULL;
  }
...
void usage(char *s)
{
  fprintf(stderr, "Usage: %s [-s|--seed <seed>] [--smoke-test]\n", s);
  exit(1);
}
...
int main(int argc, char *argv[])
{
  ...
  if (argc > 1) {
    for (i = 1, long_arg = 0; i < argc; i++, long_arg = 0) {
      if (argv[i][0] == '-') {
        ...
        if (long_arg && !strcmp(argv[i], "-smoke-test")) {
          smoke_test = 1;
          continue;
        }
        if ((!long_arg && argv[i][1] == 'h' && !argv[i][2]) ||
            (long_arg && !strcmp(argv[i], "-help"))) {
          printf("Usage: %s [-s|--seed <seed>] [--smoke-test]\n", argv[0]);
          return 0;
        }
        switch (argv[i][1]) {
        case 's':
  ...
  if (smoke_test) {
    ...
    for (int test = 0; test < 100; test++) {
      ...
      if (!test_pokemon.get_species()[0] ||
        ...
        fprintf(stderr, "Smoke test failed: invalid Pokemon data.\n");
        return 1;
      }
    }
    printf("Smoke test passed: generated 100 valid Pokemon.\n");
    return 0;
  }
  io_init_terminal();
  init_world();
  ...
  if(world.quit > 1) {
    printf("All your Pokemon got knocked out. You lost the game...\n");
  }
